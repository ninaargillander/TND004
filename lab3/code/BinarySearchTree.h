    ...
    *****************************************************/
    ...
    Node *clone(Node *t, Node* parent = nullptr) const {
        if (t == nullptr) {
            ...
            Node* ptr = new Node{ t->element, nullptr, nullptr, parent};
            ptr->left = clone(t->left, ptr);
            ptr->right = clone(t->right, ptr);

            return ptr;//new Node{t->element, clone(t->left, parent), clone(t->right, parent), parent};
        }
    }
};
